...
class BotConfig:
    ...
    # Despite the singular environment-variable
    # name, TEST_GUILD_ID may contain multiple
    # comma-separated Discord guild IDs.
    test_guild_id: tuple[int, ...] = ()

    webui_enabled: bool = False
    webui_host: str = "127.0.0.1"
    webui_port: int = 5050

    webui_credentials: tuple[
        WebUiCredential,
        ...,
    ] = ()
    ...
    webui_discord_allowed_role_ids: tuple[
        int,
        ...,
    ] = ()

    webui_discord_owner_role_ids: tuple[
        int,
        ...,
    ] = ()

    webui_discord_viewer_role_ids: tuple[
        int,
        ...,
    ] = ()
    ...
    def from_environment() -> "BotConfig":
        ...
        TEST_GUILD_ID may contain one or more
        comma-separated guild IDs. When it is
        set, slash commands are published only
        to those guilds.
        """
        token = os.getenv(
            "DISCORD_TOKEN"
        ...
        test_guild_id = parse_int_list(
            os.getenv(
                "TEST_GUILD_ID"
            )
        )

        webui_enabled = env_bool(
            "WEBUI_ENABLED",
            False,
        )

        webui_host = os.getenv(
            "WEBUI_HOST",
            "127.0.0.1",
        )
        ...
        # Only the first two numbered Web UI
        # accounts are read.
        for index in range(
            1,
            3,
            ...
            username = os.getenv(
                f"WEBUI_USER_{index}_USERNAME"
            )

            password = os.getenv(
                f"WEBUI_USER_{index}_PASSWORD"
            )
        ...
        # Discord login needs the OAuth
        # client and the guild to check.
        if (
            webui_enabled
            and webui_discord_auth_enabled
        ):
            missing_discord_settings: list[
                str
            ] = []
            ...
            if missing_discord_settings:
                ...
                raise RuntimeError(
                    "Discord WebUI login is enabled, "
                    "but these settings are missing: "
                    f"{joined}."
                )

        # With Discord auth off, password
        # login must actually be usable.
        if (
            webui_enabled
            and not webui_discord_auth_enabled
            and (
                not webui_password_login_enabled
                or not webui_credentials
            )
        ):
            raise RuntimeError(
                "WEBUI_ENABLED is true, but "
                "no WebUI login method is configured."
            )
