...
def get_current_settings(
    ...
    settings_store = context.guild_settings_store()
    ...
    return {
        ...
        "log_channel_id": str(
            settings_store.get_application_log_channel_id(
                guild.id
            )
            or ""
        ),
        "verification_form_key": (
            settings_store.get_verification_form_key(
                guild.id
            )
            or FORM_KEY_VERIFICATION
        ),
        "approved_add_role_id": str(
            settings_store.get_approved_add_role_id(
                guild.id
            )
            or ""
        ),
        "approved_remove_role_id": str(
            settings_store.get_approved_remove_role_id(
                guild.id
            )
            or ""
        ),
...
async def get_guild_forms(
    ...
    context = webui_context()
    form_store = context.form_store()

    stored_forms = await form_store.list_forms(
        guild.id
    )
    ...
    if not any(
        form["key"] == FORM_KEY_VERIFICATION
        for form in forms
    ):
        forms.insert(
            0,
...
async def get_welcome_status(
    ...
    if settings.channel_id is not None:
        ...
        if isinstance(
            ...
            channel_text = (
                f"Missing channel ({settings.channel_id})"
            )
...
def parse_optional_text_channel_id(
    guild: discord.Guild,
    raw_value: str,
    label: str,
) -> int | None:
    ...
    if not isinstance(
        ...
        raise ValueError(
            f"{label} must be a text channel "
...
def render_page(
    *,
    ...
    welcome_status: dict[str, str | bool] = {
        "available": False,
        "enabled": False,
        "channel": "Unavailable",
    }
    ...
    if selected_guild is not None:
        ...
        text_channels = context.guild_text_channels(
            selected_guild
        )
        ...
        default_terms = (
            settings_store.get_default_automod_terms()
        )

        welcome_status = context.run_coro(
            get_welcome_status(
                selected_guild
            )
        )
        ...
        if request.method == "POST":
            ...
            if action in {
                ...
                if (
                    ...
                    raise RuntimeError(
                        "Type CANCEL in the confirmation "
                        "field to cancel applications."
                    )
                ...
                if not cancellation_reason:
                    cancellation_reason = (
                        "Manually cancelled from the WebUI."
                    )
                ...
                if action == "cancel_by_user":
                    ...
                    if not user_id_text:
                        raise RuntimeError(
                            "Enter a user ID to cancel by user."
                        )
                ...
                invite_tracker = (
                    context.invite_tracker_store()
                )

                refreshed = context.run_coro(
                    invite_tracker.sync_guild_invites(
                        selected_guild
                    )
                )

                if refreshed:
                    message = "Invite cache refreshed."
                else:
                    message = (
                        "Could not refresh invites. "
                        "Check the bot has Manage Server."
                    )

                context.audit(
                    action="verification.refresh_invites",
                    guild_id=selected_guild.id,
                    detail=message,
                )

            else:
                review_channel_id = parse_optional_text_channel_id(
                    selected_guild,
                    request.form.get(
                        "review_channel_id",
                        "",
                ...
                log_channel_id = parse_optional_text_channel_id(
                    selected_guild,
                    request.form.get(
                        "log_channel_id",
                        "",
                ...
                if (
                    verification_form_key
                    not in valid_form_keys
                ):
                    raise ValueError(
                        "The selected verification form "
                        "does not exist."
                    )

                approved_add_role_id = parse_optional_role_id(
                    selected_guild,
                    request.form.get(
                ...
                approved_remove_role_id = parse_optional_role_id(
                    selected_guild,
                    request.form.get(
                ...
                if review_channel_id is None:
                    settings_store.clear_review_channel_id(
                        selected_guild.id
                    )
                else:
                ...
                if log_channel_id is None:
                    settings_store.clear_application_log_channel_id(
                        selected_guild.id
                    )
                else:
                ...
                if approved_add_role_id is None:
                ...
                if approved_remove_role_id is None:
                ...
                if action == "clear_automod_terms":
                    ...
                    message = (
                        "Verification settings saved and "
                        "automod terms cleared."
                    )
                else:
                    terms = parse_terms_from_text(
                        request.form.get(
                            "automod_terms",
                            "",
                        )
                    ...
                    if action == "add_default_terms":
                        added_count = (
                            settings_store.add_default_automod_terms(
                                selected_guild.id
                            )
                        )
                        message = (
                            "Verification settings saved. "
                            f"Added {added_count} default "
                            "automod term(s)."
                        )
                    else:
                        message = (
                ...
                if action == "save_and_post_panel":
                    panel_channel_id = parse_optional_text_channel_id(
                        selected_guild,
                        request.form.get(
                            "panel_channel_id",
                            "",
                    ...
                    panel_message = context.run_coro(
                    ...
                    message = (
                        "Verification settings saved and "
                        f"panel posted: {panel_message.jump_url}"
                    )

                context.audit(
                    action="verification.settings.save",
                    guild_id=selected_guild.id,
                    detail=message or "Saved verification settings.",
                )
